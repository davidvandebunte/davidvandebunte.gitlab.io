...
very simple: there can be at most one morphism between any two objects. Here we
...
\label{eqn.mon_prod_finset}
    ...
    \begin{cases}
      ...
      m'+g(i-m) &\mbox{ if }m+1\leq i \leq m+n.
    \end{cases}
    \qedhere
  \end{equation}
\end{example}
...
transitive closure of $R$ is called the free preorder on $R$. Rather than
...
$(f,g)$, where $f\colon V \to \Ob(\cat{C})$ and $g\colon A\to\Mor(\cat{C})$ for which $\dom(g(a))=f(s(a))$ and $\cod(g(a))=f(t(a))$ for all $a$.
	\item Is $(\Ob(\cat{C}),\Mor(\cat{C}),\dom,\cod)$ a graph? If so, see if you can use the word ``adjunction'' in a sentence that describes the statement in part 2. If not, explain why not.
\qedhere
\end{enumerate}
\end{exercise}
