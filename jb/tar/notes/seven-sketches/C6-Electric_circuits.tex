...
In \cref{def.colimit}, cocones in $\cat{C}$ are defined to be cones in $\cat{C}\op$. For visualization purposes, if $D\colon\cat{J}\to\cat{C}$ looks like the diagram to the left, then a cocone on it is shown in the diagram to the right:
\[
\boxCD{
...
\end{equation}
where $\swap{1em}$ is the symmetry on $X \otimes X$. A cocommutative comonoid
...
\[
  \elec(V)\coloneqq\{(V,A,s,t,\ell)\,|\,\mbox{ where }s,t \colon A \to V, \ell\colon A \to
C\}.
\]
...
Write $x$ for the open circuit in \eqref{eqn.circuit}. Also define cospans $\eta\colon 0\to 2$ and $\epsilon\colon 2\to 0$ as follows:
\[
\eta \coloneqq
\quad
\begin{aligned}
...
interest---we saw them as database instances in \cref{chap.databases}---so too are $\smset$-valued functors $\cat{O}\to\smset$ from any operad $\cat{O}$.%
