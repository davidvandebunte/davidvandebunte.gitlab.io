...
some of them preserve the sum of numbers, etc. Let's check in with an exercise; a solution can be found in \cref{chap.solutions}.
...
\label{exc.pullback_upset}
  ...
  Viewing upper sets as monotone maps to $\bb$ as in \cref{prop.upperset_presheaf}, the pullback can be understood in terms of composition. Indeed, show
...
between the preorder of $S$-partitions and the preorder of $T$-partitions. The way
...
Now suppose that \cref{eqn.preorder_monad_comonad} holds for all $p\in P$ and $q\in Q$. We want to show that $f(p)\leq q$ iff $p\leq g(q)$. Suppose $f(p)\leq q$; then since $g$ is monotonic, $g(f(p))\leq g(q)$, but $p\leq g(f(p))$ so $p\leq g(q)$. The proof that $p\leq g(q)$ implies $f(p)\leq q$ is similar.
\end{proof}
...
\begin{enumerate}
	...
	\item if \cref{eqn.preorder_monad_comonad} holds, then $p\leq g(q)$ iff $f(p)\leq q$ holds, for all $p\in P$ and $q\in Q$.
\qedhere
\end{enumerate}
\end{exercise}
...
By \cref{prop.galois_monad_comonad}, it suffices to show that $p_0\leq g(f(p_0))$ and that $f(g(q_0))\leq q_0$ for all $p_0\in P$ and $q_0\in Q$. For the first, we have
\[
  p_0\leq
