...
<!-- ![x](raster/2023-10-24T17-52-25.png) delete-me -->
<!-- ![x](raster/2023-10-24T17-52-59.png) delete-me -->
...
>
> The basic idea is that rather than study spaces, we should study what happens *on* spaces. A space is merely the ‘site’ at which things happen. For example, if we think of the plane $ℝ^2$ as a space, we might examine only points and regions in it. But if we
>think of $ℝ^2$ as a site where things happen, then we might think of things like weather systems throughout the plane, or sand dunes, or trajectories and flows of material. There are many sorts of things that can happen on a space, and these are the sheaves: a sheaf on a space is roughly “a sort of thing that can happen on the space.” If we want to think about points or regions from the sheaf perspective, we would consider them as different points of view on what’s happening. That is, it’s all about what happens
>on a space: the parts of the space are just perspectives from which to watch the show.
>
> This is reminiscent of databases. The schema of a database is not the interesting part; the data is what’s interesting. To be clear, the schema of a database is a site - it’s acting like the space - and the category of all instances on it is a topos. In general, we can think of any small category $𝓒$ as a site; the corresponding topos is the category of
>functors $𝓒^{op} → \textbf{Set}$⁵. Such functors are called presheaves on 𝓒.

> ⁵The category of functors $𝓒 → \textbf{Set}$ is also a topos: use $𝓒^{op}$ as the defining site.

> Did you notice that we just introduced a huge class of toposes? For any category 𝓒, we said there is a topos of presheaves on it. So before we go on to sheaves, let’s discuss this preliminary topic of presheaves. We will begin to develop some terminology and
...
<!-- ![x](raster/2023-10-24T17-53-30.png) delete-me -->
...
<!-- ![x](raster/2023-10-24T17-56-25.png) delete-me -->

> Let $𝓒$ be a small category. A *presheaf* $P$ on $𝓒$ is a functor $P: 𝓒^{op} → \textbf{Set}$. To each object $c ∈ 𝓒$, we refer to the set $P(c)$ as the *set of sections of $P$ over $c$*. To each morphism $f: c' → c$, we refer to the function $P(f) : P(c) → P(c')$ as the *restriction map along $f$*. For any section $s ∈ P(c)$, we may denote $P(f)(s) ∈ 𝑃(c')$, i.e. its restriction along $f$, by $s|_f$.
>
> If 𝑃 and 𝑄 are presheaves, a morphism 𝛼: 𝑃 → 𝑄 between them is a natural transformation of functors:
>
> ![x](presheaf-natural-transformation.svg)
...
Notice however that we can define a "contravariant functor $F: C → D$" as either a "functor from $F: C^{op} → D$" (the primary option in [Contravariant functor](https://en.wikipedia.org/wiki/Functor#Covariance_and_contravariance), and in Definition 7.22) or a "functor from $F: C → D^{op}$" (an alternative in [Contravariant functor](https://en.wikipedia.org/wiki/Functor#Covariance_and_contravariance)). Following the Wikipedia article, we'll generally prefer the first definition.
...
See also [Ball (mathematics)](https://en.wikipedia.org/wiki/Ball_(mathematics)). Notice this definition uses strict inequality `<` rather than the more familiar `≤` we use in preorders; the latter would be used for closed sets.
...
The definition of an [Open set](https://en.wikipedia.org/wiki/Open_set) on a topological space requires that the union of any two open sets be an open set: e.g. $(1,2) ∪ (5,6)$ is an open set on the real line. A topological space is a generalization of a [Metric space](https://en.wikipedia.org/wiki/Metric_space), which is again a generalization of a [Euclidean space](https://en.wikipedia.org/wiki/Euclidean_space), in which the basis of the topology is all [open balls](https://en.wikipedia.org/wiki/Ball_(mathematics)). From [Open set](https://en.wikipedia.org/wiki/Open_set):

> Equivalently, a subset $U$ of $\textbf{R}^n$ is open if every point in $U$ is the center of an [open ball](https://en.wikipedia.org/wiki/Open_ball "Open ball") contained in $U$.

By this definition $(1,2) ∪ (5,6)$ is an open set because there are open lines around every point; e.g. 1.1 has the open ball $(1,1.2)$ and 5.1 has the open ball $(5,5.2)$. This isn't clear from the author's picture, but is in the pictures on e.g. [Connected space](https://en.wikipedia.org/wiki/Connected_space).
