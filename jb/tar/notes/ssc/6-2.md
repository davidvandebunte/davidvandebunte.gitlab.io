...
See [Pushout (category theory)](https://en.wikipedia.org/w/index.php?title=Pushout_%28category_theory%29#Examples_of_pushouts), in particular the whole paragraph starting:
...
The author likely meant to write $x': X'→T$ rather than $x': X→T$.

How do we know that $x'$ is unique? Presumably because we were able to show it factors out of both $a$ and $x$.
...
The advantage to flipping both the arrows and the diagram is that the arrows end up in the same direction, from top to bottom. The disadvantages is that these drawings aren't going to make intuitive sense if you're using the word [vertex](https://en.wiktionary.org/wiki/vertex) (or less preferably [apex](https://en.wiktionary.org/wiki/apex), see more comments below), which imply the vertex is at the top of a diagram. It may be reasonable to use the terms "covertex" and "cobase" instead.
...
Notice the term "apex" is also used in [Cone (category theory)](https://en.wikipedia.org/wiki/Cone_(category_theory)); it may be better to use vertex and base (an alternative term suggested in that article) in that context to avoid an overload with this terminology. That is, use the terms apex/feet with a span/cospan, and vertex/base with a cone/cocone.
